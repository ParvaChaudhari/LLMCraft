...
export default function ConditionalNode({ id, data, selected }: { id: string, data: any, selected?: boolean }) {
  return (
    <div className="relative group" style={{ width: 192, height: 96 }}>
      ...
      <Handle type="source" position={Position.Top} id="false" className="w-2 h-2 bg-red-500 border-none rounded-full z-10" style={{ right: 55, left: 'auto', top: 25, transform: 'none' }} />
    </div>
  );
}
