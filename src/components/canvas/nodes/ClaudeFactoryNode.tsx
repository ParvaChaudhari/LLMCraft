...
export default function ClaudeFactoryNode({ id, data, selected }: { id: string, data: any, selected?: boolean }) {
  ...
  return (
    <div className="relative group" style={{ width: 192, height: 96 }}>
      ...
      <Handle type="target" position={Position.Left} id="main" className="w-2 h-2 bg-[#d97757] border-none rounded-full z-10" style={{ left: 48, top: 64 }} />
      ...
      <div className="absolute pointer-events-none" style={{ left: 0, bottom: -50, width: '100%', height: 256, transformOrigin: 'bottom center' }}>
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-16 h-16 bg-[#d97757] opacity-20 rounded-full blur-xl animate-pulse pointer-events-none -z-10"></div>
      ...
      <Handle type="source" position={Position.Right} id="output" className="w-2 h-2 bg-[#d97757] border-none rounded-full z-10" style={{ right: 32, top: 48 }} />
    </div>
  );
}
