...
export default function ChatGPTFactoryNode({ id, data, selected }: { id: string, data: any, selected?: boolean }) {
  ...
  return (
    <div className="relative group" style={{ width: 192, height: 96 }}>
      ...
      <Handle type="target" position={Position.Left} id="main" className="w-2 h-2 bg-[#74aa9c] border-none rounded-full z-10" style={{ left: 48, top: 64 }} />
      ...
      <Handle type="source" position={Position.Right} id="output" className="w-2 h-2 bg-[#74aa9c] border-none rounded-full z-10" style={{ right: 32, top: 48 }} />
    </div>
  );
}
